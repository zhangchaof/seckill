...
public class AccessInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
        ...
        if (handler instanceof HandlerMethod) {
            SeckillUserVO user = getUser(request, response);
            ...
            String concatKey = RedisConstant.ACCESS.concat(key);
            Object countObj = redisService.get(concatKey);
            Integer count = countObj == null ? null : Integer.valueOf(countObj.toString());
            if (count == null) {
                redisService.set(concatKey, 1, seconds);
            } else if (count < maxCount) {
                redisService.incr(concatKey, 1);
            } else {
    ...
    private SeckillUserVO getUser(HttpServletRequest request, HttpServletResponse response) {
